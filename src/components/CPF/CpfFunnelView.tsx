...
export const CpfFunnelView: React.FC = () => {
  ...
  const fetchCpfData = async () => {
    ...
    try {
      const [statsRes, candidatesRes] = await Promise.all([
        apiRequest<any>(`/cpf/count`),
        apiRequest<any>(`/cpf?all=true`),
      ]);
      if (statsRes) {
        setStats({
          ready: statsRes.ready ?? 0,
          promoted: statsRes.promoted ?? 0,
          nearMiss: statsRes.near_miss ?? statsRes.nearMiss ?? 0,
          low: statsRes.low ?? 0,
        });
      }
  ...
  const handlePromote = async (candidateId: string, e?: React.MouseEvent) => {
    ...
    try {
      const res = await apiRequest<any>(`/cpf/promote`, {
        method: 'POST',
        body: JSON.stringify({ candidate_id: candidateId, candidateId, id: candidateId }),
      });
      if (res?.ok || res?.success) {
        addActivityLog('CPF_PROMOTE', `Promoted candidate ${candidateId} to requirement pipeline`);
        await fetchCpfData();
        if (selectedCandidate?.id === candidateId && res.candidate) {
