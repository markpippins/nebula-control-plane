...
3. UNIMPLEMENTED ROUTING: Explicitly call out components making apiRequests to paths that do not exist or differ from definitions in $PORT_FILE (the mock-mode Express server) or the real nebula-srv REST API (live mode).
...
EOF
    ...
    git diff HEAD origin/main -U3 -- "$FRONTEND_DIR/**/*.tsx" | grep -A 3 -B 3 "services/api" >> "$AUDITOR_PAYLOAD" || echo "No layout or client apiRequest alterations found." >> "$AUDITOR_PAYLOAD"
    echo -e "\`\`\`\n" >> "$AUDITOR_PAYLOAD"

    # 5. Cross-reference against the nebula-srv REST API (live-mode ground truth).
    # The mock-mode server in $PORT_FILE is for Google AI Studio fallback; in live
    # mode every /api request is proxied to nebula-srv (:3101, PostgreSQL-backed).
    echo -e "### 4. LIVE-MODE BACKEND REFERENCE (nebula-srv REST API)\n" >> "$AUDITOR_PAYLOAD"
    echo -e "nebula-srv source: \`nexus/typescript/nebula-srv/src/routes.ts\`" >> "$AUDITOR_PAYLOAD"
    echo -e "Routes are registered as \`router.get/post/put/delete('/api/…')\` — proxy passes paths verbatim.\n" >> "$AUDITOR_PAYLOAD"
    echo -e "See DRIFT_REPORT.md for the per-endpoint delta matrix (envelope shapes, status codes, missing routes).\n" >> "$AUDITOR_PAYLOAD"
...
# 7. HEADLESS LOCAL CONFIGURATION PREPARATION
if [ -f "$PORT_FILE" ]; then
    echo "Adapting local port configurations inside $PORT_FILE..."
    # server.ts uses mode-based ports: mock -> 3000, live -> 4014.
    # Rewrite the live-mode default to TARGET_PORT (env PORT still overrides).
    sed -i "s/process.env.PORT || '4014'/process.env.PORT || '$TARGET_PORT'/g" "$PORT_FILE"
fi
